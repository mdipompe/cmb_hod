...
;     b=mhalo2bias(log_mhalo,redshift,power_spec=power_spec,delta=delta,silent=silent)
;     f=term1*term2*term3
;     alpha=1./int_tabulated(nu,f*b,/double)
;     f=alpha*f
;     print,int_tabulated(nu,f*b,/double)
  ENDIF
  ...
  IF (model EQ 'smt') THEN BEGIN
     ...
     term1=(2d*A)/nu
     term2=((1d)+((1d)/(nuprime^(2d*q))))
     term3=((nuprime^2d)/(2d*!dpi))^(1d/2d)
     term4=EXP((-1d)*(nuprime^2d)/2d)
